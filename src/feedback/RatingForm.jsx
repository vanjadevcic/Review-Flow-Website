...
const POSITIVE_STATES = { IDLE: 'idle', SENDING: 'sending', REDIRECTING: 'redirecting' }
const MAKE_WEBHOOK_URL = "https://hook.eu1.make.com/ufl5moof8kmuy65xbxca071mt6wgclal"
const POSITIVE_TIMEOUT_MS = 2500
...
export default function RatingForm({ config, clientId, locationId, LangSwitcher }) {
  ...
  // Positive flow state
  const [positiveStatus, setPositiveStatus] = useState(POSITIVE_STATES.IDLE)
  const positiveSentRef = useRef(false)
  const redirectedRef = useRef(false)

  const handlePositiveRating = useCallback(async (selectedRating) => {
    if (positiveSentRef.current) return
    positiveSentRef.current = true
    setPositiveStatus(POSITIVE_STATES.SENDING)

    const controller = new AbortController()
    const timeout = setTimeout(() => controller.abort(), POSITIVE_TIMEOUT_MS)
    ...
    try {
      ...
      // Timeout or network error — proceed anyway to maximize conversion
    } finally {
      clearTimeout(timeout)
      setPositiveStatus(POSITIVE_STATES.REDIRECTING)
    }
  }, [clientId, locationId])

  // Auto-redirect after reaching REDIRECTING state
  useEffect(() => {
    if (positiveStatus !== POSITIVE_STATES.REDIRECTING) return
    if (redirectedRef.current) return

    const timer = setTimeout(() => {
      if (redirectedRef.current) return
      redirectedRef.current = true
      window.location.href = config.google_review_link
    }, REDIRECT_DELAY_MS)

    return () => clearTimeout(timer)
  }, [positiveStatus, config.google_review_link])
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-center space-y-4">
        ...
        <StarRating rating={rating} onRate={handleRate} starAriaLabel={t.starAriaLabel} />
      </div>

      {/* Positive rating → redirect screen */}
      {rating >= 4 && positiveStatus !== POSITIVE_STATES.IDLE && (
        <div className="text-center py-8 animate-fade-in">
          <svg className="w-8 h-8 text-accent animate-spin mx-auto mb-4" viewBox="0 0 24 24" fill="none">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
          </svg>
          <p className="text-text-muted text-sm mb-6">{t.positiveRedirectText}</p>
          <a
            href={config.google_review_link}
            onClick={() => { redirectedRef.current = true }}
