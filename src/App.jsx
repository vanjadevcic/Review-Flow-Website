    ...
    <Section id="rezultati" className="border-t border-border/50">
      ...
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-10">
          <p className="text-sm text-accent font-semibold uppercase tracking-[0.15em] mb-3">{t.results.roiLabel}</p>
          <h3 className="text-2xl md:text-3xl font-bold text-text mb-3">{t.results.roiTitle}</h3>
          <p className="text-text-muted max-w-xl mx-auto">{t.results.roiSubtitle}</p>
        </div>
        <div className="grid md:grid-cols-2 gap-6">
          {/* Left: Cost of inaction */}
          <div className="p-6 rounded-2xl border border-warning/20 bg-warning/5 backdrop-blur-sm">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-xl bg-warning/10 flex items-center justify-center">
                <Icon.AlertTriangle className="w-5 h-5 text-warning" />
              </div>
              <p className="text-sm font-semibold text-text">{t.results.roiCostTitle}</p>
            </div>
            <p className="text-sm text-text-muted leading-relaxed">{t.results.roiCostDesc}</p>
          </div>
          {/* Right: Conservative calculation */}
          <div className="p-6 rounded-2xl border border-accent/20 bg-bg-card/80 backdrop-blur-sm">
            <div className="space-y-3 mb-5">
              <div className="flex items-center justify-between">
                <span className="text-sm text-text-muted">{t.results.roiCalcLabel1}</span>
                <span className="text-lg font-bold text-text">{t.results.roiCalcLine1}</span>
              </div>
              <div className="h-px bg-border" />
              <div className="flex items-center justify-between">
                <span className="text-sm text-text-muted">{t.results.roiCalcLabel2}</span>
                <span className="text-lg font-bold text-text">{t.results.roiCalcLine2}</span>
              </div>
              <div className="h-px bg-border" />
              <div className="flex items-center justify-between">
                <span className="text-sm text-text-muted">{t.results.roiCalcLabel3}</span>
                <span className="text-lg font-bold text-text">{t.results.roiCalcLine3}</span>
              </div>
            </div>
            <div className="p-4 rounded-xl bg-accent/10 border border-accent/30">
              <p className="text-3xl md:text-4xl font-bold text-accent text-center">{t.results.roiResult}</p>
              <p className="text-xs text-accent/70 text-center mt-1">{t.results.roiResultLabel}</p>
            </div>
          </div>
        </div>
